...
| `assets/` | Yes (path is gitignored, but the two files in it are tracked anyway) | Reference material actually used by the build scripts: `hq720.jpg` (photo reference for the Tamiya and armed-inverted-pendulum builds, see [`PENDULUM.md`](../PENDULUM.md)) and `inverted_pendulum_simulation.py`, a standalone matplotlib GIF storyboard generator — not a `bpy`/Blender script, not part of the `scripts/` inventory in `SKILLS.md`. |
| `images/` | No (untracked, gitignored) | Stale. Contains 8 PNGs; the first 4 are byte-identical duplicates of `renders/1..4-tamiya_pendulum_preview*.png`, and the other 4 (`5-8-tamiya_pendulum_precise_preview*.png`) have no counterpart anywhere else in the repo. No script writes here — `render_*` scripts all target `renders/`. Looks like a leftover output location from before `renders/` became the single PNG destination; safe to ignore or delete. |
| `anime/` | Yes (path is gitignored, but the one file in it is tracked anyway) | One file, `anime.gif`. No script in `scripts/` or `assets/` writes to this path or filename — `assets/inverted_pendulum_simulation.py` saves its output as `inverted_pendulum.gif` in whatever directory it's run from, not `anime/anime.gif`. Provenance looks manual/external (the generator script's docstring links to a note.com article and an `InvertedPendulumSim.tsx` component outside this repo). Not reproducible from anything currently in this repo. |
| `work/` | Yes (path is gitignored, but the one file in it is tracked anyway) | One file, `pendulum-spinning.blend` (plus its gitignored `.blend1` backup). Hand-edited in the Blender GUI directly (see its commit history — "Add centered camera, light, and exponential spin animation") rather than produced by a script. Scratch/manual-session space, distinct from `renders/`'s "regenerate via scripts" contract. |
