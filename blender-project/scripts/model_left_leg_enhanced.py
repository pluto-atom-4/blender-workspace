"""
Left Leg Enhanced Mechanism Model -- 5/6-bar linkage with dual actuation (issue #102)

A standalone six-bar leg assembly with dual actuation:
  - Hip servo (STS3032) drives the Drive Rod
  - Wheel-hub motor (STS3032) actuates the wheel
  - Damping spring provides compliance between calf and knee
  - Crus Side Rod acts as a rigid brace, creating a virtual pivot for shaping the wheel path

Structure:
  - Hip motor: STS3032 servo (SERVO_W/L/H = 20/40/36mm)
  - Drive Rod (driven): upper link, dual-shear construction
  - Thigh Rod1 (passive): lower link, dual-shear construction
  - Crus Side Rod: rigid brace connecting Drive Rod to Thigh Rod1
  - Knee Rod: coupler/pivot at the knee junction
  - Calf Rod1/Calf Rod2: paired dual-plate segment from knee to wheel axle
  - Damping Spring: between knee rod and calf rod1 (placeholder PEA spring pattern)
...
This follows the naming convention guideline for new mechanisms:
`model_left_leg_enhanced.py` (describing the enhanced mechanism, not a fidelity upgrade
of an existing one, per issue #102 decision 5).

Sections:
  1. Units / cleanup / materials
  2. Mesh primitives (box, cylinder)
  3. Component builders (servo, dual-shear link, spring, knee rod, crus rod, wheel hub motor)
...
COLLECTION_NAME = "LeftLegEnhanced"
PREFIX = "LLE_"
...
# Crus Side Rod (triangulation)
CRUS_ROD_LEN = 35.0       # approximate diagonal length connecting drive rod to thigh rod

# Spring dimensions (PEA spring pattern from model_dual_wheel_legged_robot_precise.py)
...
def build_pea_spring(tag, collection, sign):
    ...
    A simple representative tube standing in for a real part.
    This is a bespoke copy of the pattern from model_dual_wheel_legged_robot_precise.py
    per issue #102 decision 4.

    Spring spans from knee rod back toward the hip, running parallel to the drive rod.
    Built once at DEFAULT_HIP_ANGLE and parented to the drive rod, so it swings
    rigidly with the hip angle.

    Geometrically exact only at the rest pose; across the animation it's a static
    rigid mesh (same simplification as other FK-chain parts).

    The spring constant (N*m/rad) is computed separately in
    orchestration/linkage_kinematics.py; this is visual geometry only.
    """
...
def build_crus_side_rod(collection):
    ...
    Per issue #102 decision 3: rigid (does not add a second DOF).
    """
    crus = make_box(
...
def build_wheel(collection):
    """Treaded wheel with tread grooves."""
    r = WHEEL_R
    w = WHEEL_W
...
# ---------------------------------------------------------------------------
# 4. Assembly: complete enhanced leg mechanism
# ---------------------------------------------------------------------------

def build_assembly(collection):
    """
    Assemble the complete enhanced leg:
    - Hip servo at origin
    - Drive Rod + Thigh Rod1 + Crus Side Rod (triangulation)
    - Knee Rod coupler
    - Calf Rod1 + Calf Rod2 (paired down to wheel)
    - Damping Spring
    - Wheel hub motor + wheel
    """

    default_rad = math.radians(DEFAULT_HIP_ANGLE)

    # Hip servo mounting assembly
    ...
    # Crus Side Rod (rigid triangulation brace)
    ...
    # Damping Spring (parented to drive rod, springs between knee and calf)
    ...
    # Calf Rod 1 (dual-shear, from knee to wheel axle)
    calf_rod = build_calf_link(f"{PREFIX}CalfRod", collection, CALF_ROD_LEN)
    for key in calf_rod:
        calf_rod[key].parent = knee_rod

    # Wheel hub motor (mounted at the wheel axle, below the calf rod)
    ...
    # Wheel (actuated by wheel hub motor)
...
def main():
    ...
    if bpy.app.background:
        output_path = "/home/pluto-atom-4/blender-workspace/blender-project/renders/left_leg_enhanced.blend"
